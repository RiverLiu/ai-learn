...
| 本项目模块 | 用到的能力 | 对应教程 |
| --- | --- | --- |
| `app/knowledge.py` | 切块、向量化、余弦相似度检索 | `tutorials/rag/`（尤其 `04_rag_pipeline`） |
| `app/agent.py` | create_agent 构建 ReAct 工具调用循环 | `tutorials/langgraph/03_react_agent` |
| `app/agent.py` | checkpointer + thread_id 多轮记忆 | `tutorials/langgraph/05_persistence` |
| `app/agent.py` | @tool 工具定义 | `tutorials/langchain/`、`tutorials/tools/` |
| `app/main.py` | SSE 流式输出 token | `tutorials/llm_api/02_params_streaming` |
| `app/main.py` | FastAPI 服务形态 | `tutorials/fastapi/` |
| `scripts/eval.py` | 评估集 + LLM 评委打分 | `tutorials/evaluation/` |
...
1. 配置密钥（参考 `.env.example`）：
   ...
   ```bash
   cp tutorials/capstone/.env.example tutorials/capstone/.env
...
6. 跑小评测（5 条问答 + LLM 评委打分，输出评分表）：
   ...
   ```bash
   uv run tutorials/capstone/scripts/eval.py
   ```
...
1. **加 MCP 工具**：参考 `tutorials/mcp/`，把"查询工单状态"做成 MCP 服务，
   让 Agent 通过 MCP 协议调用，体会与本地 @tool 的差别。
2. **换本地模型**：参考 `tutorials/local_models/`，用 Ollama 把向量模型换成
...
4. **长期记忆**：参考 `tutorials/memory/`，让 Agent 跨线程记住用户偏好
...
7. **WebSocket 版本**：参考 `tutorials/fastapi/10_websocket`，把 /chat 改成
