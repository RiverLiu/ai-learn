...
# 加载 .env：优先本模块目录（tutorials/capstone/.env），其次向上查找（如项目根目录）
...
def load_chat_config() -> ChatConfig:
    ...
    if not api_key:
        raise RuntimeError("缺少 OPENAI_API_KEY，请参考 tutorials/capstone/.env.example 配置")
    return ChatConfig(
        api_key=api_key,
...
def load_embedding_config() -> EmbeddingConfig:
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "请参考 tutorials/capstone/.env.example 配置"
        )
