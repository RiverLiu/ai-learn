...
运行（在仓库根目录）：uv run tutorials/multimodal/02_audio_transcribe/main.py
"""
...
def transcribe_with_whisper_cpp(exe: str):
    ...
    if not model_path or not Path(model_path).exists():
        ...
        print("然后指定模型路径重跑本脚本：")
        print(f"  WHISPER_CPP_MODEL=/path/to/ggml-tiny.bin uv run tutorials/multimodal/02_audio_transcribe/main.py")
        return
...
def print_guide():
    ...
    print(f"装好任意一种后重跑：uv run tutorials/multimodal/02_audio_transcribe/main.py")
