...
- **调用循环四步**：模型返回 `tool_calls` → 本地 `json.loads` 参数并执行 →
  ...
  直到它不再要工具、给出最终回答。
- **这是 Agent 的最小骨架**：后面 [mcp 教程](../../mcp/)的工具协议、
  [langchain 教程](../../langchain/)的 `@tool` 装饰器，全是这套循环的封装。
...
```bash
uv run tutorials/llm_api/04_function_calling/main.py
```
...
- **假数据无碍学习**：本章工具返回固定数据，把 `get_weather` 内部换成真实 HTTP
  请求（回顾 [basic 模块](../../basic/)的 aiohttp）就是生产级天气助手。
