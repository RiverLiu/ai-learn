...
```bash
cp tutorials/llm_api/.env.example .env
```
...
```bash
uv run tutorials/llm_api/01_hello_llm/main.py
```

## 前置与后续

- 前置：Python 基础语法 + [basic 模块](../basic/)的 JSON（请求/响应都是 JSON）。
- 后续：本模块的手写调用是所有上层框架的"裸机版"。学完建议接着看
  [rag 教程](../rag/)（给模型外挂知识库，多配一个 Embedding 模型即可）与
  [langchain 教程](../langchain/)（把本章的消息、流式、工具调用封装成组件，
