...
[RAG 第 1 章](../../rag/01_embeddings/) 的语义相似度演示。
...
```bash
uv run tutorials/local_models/02_local_embedding/main.py
```
...
```bash
rm -f tutorials/rag/03_vector_store/rag_index.npz tutorials/rag/03_vector_store/rag_index.json
uv run tutorials/rag/03_vector_store/main.py   # 用 bge-m3 重建索引
```
...
2. 完成 `.env` 配置并重建索引后，运行 `uv run tutorials/rag/04_rag_pipeline/main.py`，
   体验"聊天 + 向量全本地"的完整 RAG。
