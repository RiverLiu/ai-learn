...
参考答案：它是 FastAPI + LangGraph Agent + RAG 的智能客服系统，支持 SSE 流式输出、知识库检索、工具调用和评估。

**Q2：为什么要同时配置聊天模型和向量模型？**

参考答案：聊天模型负责生成和工具决策，向量模型负责语义检索。很多服务不同时提供两类能力。

**Q3：用户反馈答案不准时如何排查？**

参考答案：先查检索命中，再查 chunk 和文档版本，然后看 prompt 和模型是否忠实使用上下文。

**Q4：SSE 在项目中负责什么？**

参考答案：负责把状态、工具结果和 token 增量推送给客户端，提升聊天体验。

**Q5：Agent 在项目中做了什么？**

参考答案：根据用户问题决定检索知识库或调用退款工具，并基于工具结果生成最终回答。

**Q6：知识库索引如何构建？**

参考答案：读取 Markdown 文档，按标题切块，调用 embedding，保存向量并用相似度检索。

**Q7：为什么要做评估脚本？**

参考答案：用固定样本和 LLM judge 检查回答质量，避免 prompt、模型或知识库修改后回归。

**Q8：这个项目的生产化缺口是什么？**

参考答案：缺少用户系统、持久化数据库、后台索引任务、权限过滤、观测、反馈闭环和部署编排。

**Q9：如何把内存索引升级为生产索引？**

参考答案：引入向量数据库、文档表、chunk metadata、增量更新、删除策略和权限过滤。

**Q10：如何说明项目亮点？**

参考答案：强调它串联了 RAG、Agent、工具调用、流式输出、记忆和评估，是完整 AI 应用闭环。
