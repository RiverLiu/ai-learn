...
| `StoreBackend()` | LangGraph Store（需配 `store=`） | 跨会话持久 | 用户档案、长期笔记（呼应 [memory 教程](../../memory/)） |
...
```bash
uv run tutorials/deepagents/04_backends/main.py
```
...
- `workspace/` 目录已加入 gitignore；Store 持久化选型见
  [langgraph 教程第 5 章](../../langgraph/05_persistence/)。
